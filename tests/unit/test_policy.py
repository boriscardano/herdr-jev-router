...
@pytest.mark.parametrize(
    "state", [CapacityState.CRITICAL, CapacityState.EXHAUSTED, CapacityState.UNKNOWN]
)
def test_decision_validation_accepts_a_launchable_harness_whatever_its_quota(
    state: CapacityState,
) -> None:
    capacities = (
        ProviderCapacity(
            ...
            reason=(
                ...
                if state is CapacityState.CRITICAL
                else None
            ),
        ),
        ProviderCapacity(Harness.CLAUDE, CapacityState.ON_PACE),
